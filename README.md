...
Detailed item-specific reproduction instructions for Figure 7 and Figure 8 are provided in Sections 8.5 and 8.6.
...
Detailed item-specific reproduction instructions for Figures 1, 6, 9, 10, and 11 are provided in Sections 8.3, 8.4, 8.7, 8.8, and 8.9.
...
Table 1 is a feature schema description table rather than a numerical result table. It summarizes the variables used as model inputs and their data types or encoding schemes. The variables listed in Table 1 are selected and processed in `For_test/code3_create_traveler_feature.ipynb`.

Specifically, traveler information, companion information, tour information, and GPS trajectory information are loaded from the simulated raw-style AI-Hub files. The notebook then selects the variables used in the route prediction models and converts them into model-ready feature columns. Categorical variables, such as marital status, occupation, travel style, start month, end month, travel purpose, and transportation mode, are converted into one-hot encoded variables. Gender is encoded as a binary variable. Numerical or ordered variables, such as age, family size, personal income, household income, travel frequency, trip duration, and companion number, are retained or converted into integer-coded variables.

The resulting feature tables are saved as `properties_for_traveler.csv` for the training and validation datasets. Table 1 documents the variable groups, feature names, and encoding schemes corresponding to these processed feature tables. Because Table 1 is a descriptive schema table, it is not generated as a separate numerical output file.
...
1. Run the full route prediction and accuracy-evaluation workflow demonstrated in `For_test`:
...
2. The same evaluation workflow was applied to the full original dataset to generate the accuracy files used for Figure 7.

3. Run `Figure_and_Real/evaluation_and_significance_fig_7_8.ipynb`.

4. Execute the section corresponding to Figure 7.
...
2. The same evaluation workflow was applied to the full original dataset to generate the accuracy files used for Figure 8.

3. Run `Figure_and_Real/evaluation_and_significance_fig_7_8.ipynb`.

4. Execute the section corresponding to Figure 8.
...
The `Figure_11/` folder contains the selected observed GPS trajectory files and the corresponding route prediction file used to reproduce Figure 11. In the current repository, the main files used for Figure 11 include:

  * `all_multi_route_generated_routes_parallel.csv`: route prediction results generated by the four route prediction models
  * `h000108_3_0.csv`: observed GPS trajectory file used as an actual route example
  * `h004703_6_0.csv`: observed GPS trajectory file used as an actual route example

The selected GPS CSV files represent the actual observed routes, while `all_multi_route_generated_routes_parallel.csv` contains the predicted routes generated by the four route prediction models.
